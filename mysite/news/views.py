...
def register(requset):
    ...
    return render(requset, 'news/register.html', {'form': form})
...
def user_login(requset):
    if requset.method == 'POST':
        ...
        form = UserLoginForm()
    return render(requset, 'news/login.html', {'form': form})
...
class HomeNews(MyMixin, ListView):
    ...
    # extra_context = {'title': 'Главная'} не желательно при динамике
...
class CreateNews(LoginRequiredMixin,
                 CreateView):  # используется redirect по умолчанию благодаря get_absolute_url из модели
    form_class = NewsForm
    template_name = 'news/add_news.html'
    # login_url = '/admin/' #если пользователь не авторизовано отправлять на вход
    raise_exception = True
    # success_url = reverse_lazy('home')  #redirect, если нет get_absolute_url
